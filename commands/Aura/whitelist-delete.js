...
.setTitle("Error")
.setDescription(`<a:no:865963806483808256> I cant find that user on database. `)
...
.setDescription(`${user} has been removed from whitelist users <a:yes:865963544380964864>`)
.setColor(colors.uptime)
return message.channel.send(deleted)
} else {          
message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.username}`, `${message.author.avatarURL({dynamic:true})}`).setDescription(`<a:no:865963806483808256> that user not on whitelist users list`) .setColor(colors.uptime))
}}
message.channel.send(new Discord.MessageEmbed().setAuthor(`${message.author.username}`, `${message.author.avatarURL({dynamic:true})}`).setDescription(`<a:no:865963806483808256> Only \`ownership\` of the guild can use that cmd!`) .setColor(colors.uptime))}}
